...
export function ImageSlider({ images, badge }: ImageSliderProps) {
  ...
  return (
    <div className="w-full px-5 md:px-20 max-w-360 mx-auto">
      <div className="grid gap-2">
        {/* Main Image */}
        <div className="relative overflow-hidden rounded-sm aspect-video max-h-145">
          <Image
          ...
          {/* Badge */}
          {badge && (
            <span className="absolute top-5 left-5 bg-[#0a0a0a] text-white font-sans text-[9px] tracking-[0.14em] uppercase px-2.5 py-1.25 font-semibold">
              {badge}
            </span>
          )}
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            {images.map((image, index) => (
              <button
                ...
                className={cn(
                  'shrink-0 w-25 h-17 overflow-hidden rounded-sm border-2 p-0 cursor-pointer transition-all',
                  index === currentIndex
                    ? 'border-[#b89a5b]'
